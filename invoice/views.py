...
from .form import WorkOrderForm, InvoiceForm,  WorkItemForm, WorkOrderForm
...
WorkOrderFormset = inlineformset_factory(WorkOrder, WorkItem, form=WorkOrderForm, fields=['cost'], extra=1)    
...
def create2(request):
    ...
    InvoiceFormSet = inlineformset_factory(Contact, Invoice, form=InvoiceForm, fields=['title', 'descript', 'deadline', 'status'], extra=1, can_delete=False)
    WorkItemFormSet = inlineformset_factory(WorkOrder, WorkItem, fields='__all__', extra=1)
    ...
    work_items = WorkItemFormSet()
    invoice = InvoiceFormSet(prefix='invoice')
